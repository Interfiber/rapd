...
pub fn get_title(path: String) -> String {
    ...
    let tag = Tag::new().read_from_path(path).expect("Failed to read music file from disk");
...
pub fn set_title(path: String, value: String) {
    ...
    let mut tag = Tag::new().read_from_path(&path).expect("Failed to read music file from disk");
    tag.set_title(&value);
    // save the metadata
